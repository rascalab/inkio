...
const ALL_ICON_IDS: InkioIconId[] = [
  ...
  'callout', 'toggleList', 'codeBlock', 'horizontalRule',
  'link', 'unlink', 'comment',
];
...
describe('inkioIconRegistry', () => {
  ...
  it('all icons are renderable (function or forwardRef object)', () => {
    ALL_ICON_IDS.forEach((id) => {
      ...
      // lucide-react exports forwardRef components which are objects with $$typeof,
      // but they are also callable via React — check they exist and are not null/undefined
...
describe('splitToolbarActionGroups', () => {
  ...
  it('single action produces single group of one', () => {
    const singleAction = [inkioToolbarSchema.find((a) => a.id === 'bold')!];
...
describe('inkioToolbarSchema', () => {
  ...
  it('each surface value is either bubble or floating', () => {
    inkioToolbarSchema.forEach((action) => {
      action.surfaces.forEach((surface) => {
        expect(['bubble', 'floating']).toContain(surface);
      });
    });
  });
});
