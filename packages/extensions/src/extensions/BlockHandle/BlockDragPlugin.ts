...
import { createElement as createLucideIcon, GripVertical } from 'lucide';
import { BlockHandleActionMenu } from './BlockHandleView';
import type { BlockMenuIcons } from './icons';
import { getCreateRoot } from '../../utils/createRoot';
...
const createHandleElement = () => {
  ...
  const icon = createLucideIcon(GripVertical, { width: 14, height: 14 });
...
export const createBlockHandlePlugin = (options: BlockHandlePluginOptions) => {
  ...
  const ensureHandleElement = () => {
    ...
    const LIST_ITEM_TYPES = new Set(['listItem', 'taskItem', 'toggleListItem']);
    const LIST_TYPES_SET = new Set(['bulletList', 'orderedList', 'taskList', 'toggleList']);
