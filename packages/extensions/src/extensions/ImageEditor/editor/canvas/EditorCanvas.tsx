...
export function EditorCanvas({
  ...
  // Scale to fit
  const scaleToFit = imgW > 0 && imgH > 0
  ...
  const getRelativePos = useCallback((_e: Konva.KonvaEventObject<MouseEvent | TouchEvent>) => {
    ...
    // getPointerPosition() already returns Stage-relative coords,
    // so no need to subtract offsetX/offsetY.
    return {
      x: pos.x / scaleToFit,
      y: pos.y / scaleToFit,
    };
  ...
  const handleMouseUp = useCallback((e: Konva.KonvaEventObject<MouseEvent | TouchEvent>) => {
    ...
    // Text tool: create annotation on mouseUp (drag defines area, or click creates default)
    if (state.activeTool === 'text' && drawStartPos.current) {
      const pos = getRelativePos(e);
      const start = drawStartPos.current;
      const dragWidth = Math.abs(pos.x - start.x);
      const width = dragWidth > 10 ? dragWidth : 120; // If just clicked (no drag), default 120px
    ...
    // Flush any pending freedraw points
    if (rafId.current) {
    ...
    if (id) {
      ...
      if (ann) {
        // Remove too-small shapes (accidental click)
        let tooSmall = false;
  ...
  return (
    ...
    >
      <div style={canvasStyle}>
        ...
        <TextEditOverlay scaleToFit={scaleToFit} />
      </div>
    </div>
  );
};
