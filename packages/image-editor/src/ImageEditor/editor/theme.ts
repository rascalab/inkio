...
/** Clear the module-level color cache. Call this in SSR environments between requests. */
export function clearIEColorCache(): void {
  _cache = null;
  _cacheTheme = undefined;
}
...
export function getIEColors(): IEColors {
  ...
  const cacheKey = `${dataTheme}:${darkClass}`;
  ...
  _cache = {
    ...
    canvasBg: getCssVar(root, '--inkio-ie-canvas-bg', '#1e1e1e'),
