...
export function FilterOptionsPanel() {
  ...
  return (
    <>
      <PresetChipGroup
        label={locale.filter}
        items={FILTER_PRESETS.map((preset) => ({
          key: preset.id,
          label: locale[preset.labelKey],
          active: state.filter === preset.id,
          onClick: () => dispatch({ type: 'SET_FILTER', filter: preset.id }),
          testId: `inkio-ie-filter-${preset.id}`,
        }))}
      />
      <RangeFieldGroup
      ...
      <button
        ...
        data-testid="inkio-ie-finetune-reset"
      >
        {locale.resetFinetune}
      </button>
    </>
  );
}
